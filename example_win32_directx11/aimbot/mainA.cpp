...
{
    const char *espaces = " \x09\x0D\x0A";

    const size_t debut = str.find_first_not_of(espaces);
    if (debut == std::string::npos)
        return {};

    const size_t fin = str.find_last_not_of(espaces);
...
int APIENTRY WinMain(HINSTANCE, HINSTANCE, LPSTR, int)
{
    ...
    while (!done)
    {
        ...
        if (var::esp)
        {
            ...
            if (var::esp_status_hud)
            {
                ...
                ImGui::GetBackgroundDrawList()->AddRectFilled(
                    ImVec2(10.0f, 10.0f), ImVec2(320.0f, 35.0f),
                    ImColor(0, 0, 0, 150), 4.0f);
                ImGui::GetBackgroundDrawList()->AddText(
                    nullptr, 14.0f, ImVec2(18.0f, 15.0f),
                    ImColor(255, 255, 255, 255), status_buf);
            }
        } // FIN ESP
        ...
        if (var::fovCircle)
        {
            ImGui::GetBackgroundDrawList()->AddCircle(
                ...
                var::fovSize, ImColor(155, 89, 182, 150), 100, 1.5f);
        }
        ...
        if (us == nullptr)
            D3DX11CreateShaderResourceViewFromMemory(g_pd3dDevice, user, sizeof(user), &info, nullptr, &us, nullptr);
        if (dr1 == nullptr)
            D3DX11CreateShaderResourceViewFromMemory(g_pd3dDevice, roll, sizeof(roll), &info, nullptr, &dr1, nullptr);
        ...
        if (open)
        {
            if (!login)
            {
                ...
                ImGui::GetWindowDrawList()->AddRectFilled(
                    ImVec2(0.0f + p.x, 0.0f + p.y),
                    ImVec2(855.0f * dpi_scale + p.x, 650.0f * dpi_scale + p.y),
                    ImGui::GetColorU32(colors::main_color), s.WindowRounding);
                ...
                ImGui::BeginChild("LoginWindow", ImVec2(400.0f * dpi_scale, 550.0f * dpi_scale), true);
                {
                    ...
                    ImGui::BeginChild("LoginContent", ImVec2(360.0f * dpi_scale, 400.0f * dpi_scale), false);
                    {
                        if (login_tab == 0)
                        {
                            ...
                            if (ImGui::Button("Login##btn", ImVec2(160.0f * dpi_scale, 40.0f * dpi_scale)))
                            {
                                KeyAuthApp.init();
                        ...
                        else
                        {
                            ...
                            if (ImGui::Button("Register##btn", ImVec2(160.0f * dpi_scale, 40.0f * dpi_scale)))
                            {
                                KeyAuthApp.init();
                    ...
                    ImGui::EndChild(); // LoginContent
                }
                ImGui::EndChild(); // LoginWindow
                ImGui::End();      // Login
            }
            else if (login)
            {
                // ====== MAIN MENU ======
                ImGui::SetNextWindowSize(ImVec2(855.0f * dpi_scale, 790.0f * dpi_scale));
                ...
                ImGui::GetWindowDrawList()->AddRectFilled(
                    ImVec2(0.0f + p.x, 0.0f + p.y),
                    ImVec2(855.0f * dpi_scale + p.x, 790.0f * dpi_scale + p.y),
                    ImGui::GetColorU32(colors::main_color), s.WindowRounding);
                ImGui::GetWindowDrawList()->AddRectFilled(
                    ImVec2(0.0f + p.x, 755.0f * dpi_scale + p.y),
                    ImVec2(855.0f * dpi_scale + p.x, 755.0f * dpi_scale + p.y),
                    ImGui::GetColorU32(colors::lite_color), s.WindowRounding);

                ImGui::PushStyleColor(ImGuiCol_ChildBg, ImVec4(0, 0, 0, 0));
                ImGui::BeginChild("G-Tab", ImVec2(173.0f * dpi_scale, 790.0f * dpi_scale), false);
                {
                    ImGui::GetForegroundDrawList()->AddText(tab_text3, 20.0f * dpi_scale,
                                                            ImVec2(20.0f * dpi_scale + p.x, 12.0f * dpi_scale + p.y),
                                                            ImColor(255, 255, 255, 255), " mk cheat");
                    ImGui::GetWindowDrawList()->AddRectFilled(
                        ImVec2(0.0f + p.x, 0.0f + p.y),
                        ImVec2(273.0f * dpi_scale + p.x, 790.0f * dpi_scale + p.y),
                        ImGui::GetColorU32(colors::Tab_Child), s.WindowRounding);
                ...
                ImGui::BeginChild("General", ImVec2(717.0f * dpi_scale, 650.0f * dpi_scale), false);
                {
                    ...
                    case 1:
                    {
                        ...
                        ImGui::BeginChildPos("Misc", ImVec2(620.0f * dpi_scale, 580.0f * dpi_scale));
                        {
                            ...
                            ImGui::SliderFloat("Confidence Threshold", &var::confidenceThreshold, 0.1f, 1.0f, "%.2f");
                            ImGui::Spacing();

                            const char *backends[] = {"CPU", "GPU (CUDA)", "TensorRT"};
                            int backend_idx = 0;
                            if (var::detection_backend == "GPU (CUDA)")
                                backend_idx = 1;
                            else if (var::detection_backend == "TensorRT")
                                backend_idx = 2;
                            if (ImGui::Combo("Detection Backend", &backend_idx, backends, IM_ARRAYSIZE(backends)))
                            {
                                var::detection_backend = backends[backend_idx];
                            }

                            ImGui::Spacing();
                            ImGui::Separator();
                            ImGui::Spacing();
                            ImGui::TextColored(ImVec4(0.55f, 0.35f, 0.95f, 1.00f), "OFFSETS");
                            ImGui::Separator();
                            ImGui::Spacing();
                            ImGui::SliderFloat("Offset X", &var::offsetX, -200.0f, 200.0f, "%.1f");
                            ImGui::SliderFloat("Offset Y", &var::offsetY, -200.0f, 200.0f, "%.1f");
                            ImGui::SliderFloat("Scale", &var::scale, 0.5f, 2.0f, "%.2f");

                            ImGui::Spacing();
                            ImGui::Separator();
                            ImGui::Spacing();
                            ImGui::Checkbox("Debug Console", &var::debug_console);
                            ImGui::Keybind("Toggle Menu Key", &var::key4, true);
                        }
                        ImGui::EndChild();
                    }
                    break;
                    ...
                    ImGui::PopStyleColor(); // lite_color
                }
                ImGui::EndChild();      // General
                ImGui::PopStyleColor(); // transparent
    ...
    CleanupDeviceD3D();
    ::DestroyWindow(hwnd);
    ::UnregisterClassA("Example", nullptr);
    return 0;
}
